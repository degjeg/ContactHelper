...
public class SQLiteDatabasePool {
    private static final String TAG = "TASQLiteDatabasePool";
    ...
    private ESQLiteDatabase.TADBParams params;
    private ESQLiteDatabase.TADBUpdateListener mDBUpdateListener; // 升级时监听器
    ...
    public synchronized static SQLiteDatabasePool getInstance(
            Context context, ESQLiteDatabase.TADBParams params, Boolean isWrite) {
    ...
    public static SQLiteDatabasePool getInstance(Context context) {
        ESQLiteDatabase.TADBParams params = new ESQLiteDatabase.TADBParams();
    ...
    public static SQLiteDatabasePool getInstance(Context context,
        ...
        ESQLiteDatabase.TADBParams params = new ESQLiteDatabase.TADBParams(dbName, dbVersion);
     ...
     * @param isWrite 打开数据库如果是 isWrite为true,则磁盘满时抛出错误
     */
    public SQLiteDatabasePool(Context context, ESQLiteDatabase.TADBParams params,
                              Boolean isWrite) {
     ...
     */
    public void setOnDbUpdateListener(ESQLiteDatabase.TADBUpdateListener dbUpdateListener) {
    ...
    private void createSQLiteDatabase(int numSQLiteDatabase) {
        ...
        for (int x = 0; x < numSQLiteDatabase; x++) {
            ...
            try {
                ...
                if (DBHelper.logOn) {
                    LogUtil.i(TAG, " 创建数据库连接失败！ " + e.getMessage());
                }
            }

            if (DBHelper.logOn) {
                LogUtil.i(TAG, "数据库连接己创建 ......");
            }
        }
    }
    ...
    private ESQLiteDatabase newSQLiteDatabase() {

        // 创建一个数据库连接
        ESQLiteDatabase sqliteDatabase = new ESQLiteDatabase(context, params);
    ...
    public synchronized ESQLiteDatabase getSQLiteDatabase() {
        // 确保连接池己被创建
        if (pSQLiteDatabases == null) {
            return null; // 连接池还没创建，则返回 null
        }

        ESQLiteDatabase sqliteDatabase = getFreeSQLiteDatabase(); // 获得一个可用的数据库连接
    ...
    private ESQLiteDatabase getFreeSQLiteDatabase() {
        // 从连接池中获得一个可用的数据库连接
        ESQLiteDatabase sqLiteDatabase = findFreeSQLiteDatabase();
    ...
    private ESQLiteDatabase findFreeSQLiteDatabase() {
        ESQLiteDatabase sqliteDatabase = null;
     ...
     */
    private boolean testSQLiteDatabase(ESQLiteDatabase sqliteDatabase) {
    ...
    public void releaseSQLiteDatabase(ESQLiteDatabase sqLiteDatabase) {
        // 确保连接池存在，如果连接没有创建（不存在），直接返回
        if (pSQLiteDatabases == null) {
            if (DBHelper.logOn) {
                LogUtil.d(TAG, " 连接池不存在，无法返回此连接到连接池中 !");
            }
            return;
        }
        PooledSQLiteDatabase pSqLiteDatabase = null;
    ...
    private void closeSQLiteDatabase(ESQLiteDatabase sqlLiteDatabase) {
    ...
    class PooledSQLiteDatabase {
        ESQLiteDatabase sqliteDatabase = null;// 数据库连接
        boolean busy = false; // 此连接是否正在使用的标志，默认没有正在使用

        // 构造函数，根据一个 TASQLiteDatabase 构告一个 PooledSQLiteDatabase 对象
        public PooledSQLiteDatabase(ESQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }

        // 返回此对象中的连接
        public ESQLiteDatabase getSqliteDatabase() {
            return sqliteDatabase;
        }

        // 设置此对象的，连接
        public void setSqliteDatabase(ESQLiteDatabase sqliteDatabase) {
            this.sqliteDatabase = sqliteDatabase;
        }
